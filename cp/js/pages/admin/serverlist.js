        ...
        <div class="ms-3">
            <div class="user-meta-info">
                <h6 class="user-name mb-0">${e.name}</h6>
                    ...
                    `;var s=i18next.t("checkGuild.embed.fields",{returnObjects:!0});for(const i in t)t.hasOwnProperty(i)&&(n+=`
                            <div class="row">
                                <label for="${s[i].name}" class="col-lg-2 form-label">${s[i].name}</label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" id="${s[i].name}" value="${t[i]}" readonly>
                                </div>
                            </div>`);a.banner&&(n+=`
                        <div class="row">
                            <img src="${a.banner.url}">
                        </div>`),r.innerHTML=n,document.getElementById("editform").setAttribute("action","/admin/serverlist/"+e.id),o.innerHTML=`
                    ...
                    `},error:function(e){window.NotificationHandler.show({content:e.responseJSON.error,type:"error"}),r.innerHTML=window.ContentHandler.createAlert(e.responseJSON.error),o.innerHTML=`<button type="button" class="btn btn-dark" data-bs-dismiss="modal">${i18next.t("dashboard.cancel")}</button>`}}))}),a.addEventListener("hidden.bs.modal",e=>{document.getElementsByClassName("modalactive")[0].classList.remove("active","modalactive")})})();