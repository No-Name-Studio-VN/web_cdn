(()=>{window.InjectCSS(`.card .blog {
    ...
    cursor: pointer;
  }`);const s=document.getElementById("listHolder"),t=document.getElementById("listHolderLoading");$.ajax({url:"/api/blog/list",method:"GET",contentType:"application/json",success:e=>{e.data.forEach(async(e,t)=>{t=await function(e,t){var{meta:t,href:s}=t,i=JSON.parse(t.tags),i=t.tags?i[0]:"Blog",a=t.avatar||`https://cdn.discordapp.com/embed/avatars/${Math.floor(6*Math.random())}.png`,d=t.author||"No Name Studio",l=t.banner||`/img/blogs/img${Math.floor(10*Math.random())+1}.jpg`,o=t.title;let r=t.description||null;60<r?.length&&(r=r.slice(0,60)+"...");var n=`${t.readtime} min${1<t.readtime?"s":""} read`,t=t.modifiedTimestamp,c=document.createElement("div");c.classList.add("col-md-6","mb-3"),e<2?c.classList.add(0===e?"col-lg-8":"col-lg-4"):c.classList.add("col-lg-4","d-flex","align-items-stretch");c.setAttribute("nns-redirect",""),c.setAttribute("nns-redirect-data",s);let m;m=e<2?`
