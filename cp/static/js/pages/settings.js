(()=>{let l={desktop:"ti ti-device-laptop",mobile:"ti ti-device-mobile",tablet:"ti ti-device-ipad",default:"ti ti-device-tv"};$("#pills-security-tab").one("show.bs.tab",function(){LoadHandler.js("https://cdn.jsdelivr.net/npm/ua-parser-js/src/ua-parser.min.js").then(()=>{$.ajax({url:"/auth/sessions",method:"GET",contentType:"application/json",success:function(e){var s=e.user_sessions;let i=$("#sessions_box");if(i.empty(),0===s.length)i.append('<p class="text-center">No sessions found</p>');else{var n=new UAParser;i.append(`
