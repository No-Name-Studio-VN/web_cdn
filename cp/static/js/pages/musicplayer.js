(()=>{window.AnimateBackground({light:"repeating-linear-gradient(45deg, #92c9b1, #92c9b1 20px, #b3e0d2 20px, #b3e0d2 40px)",dark:"repeating-linear-gradient(45deg, #92c9b1, #92c9b1 20px, #b3e0d2 20px, #b3e0d2 40px)",duration:"30s"});let s=$("#lyrics-content").find(".simplebar-content"),t=0,l=!1,r=!1;function o(e){e.empty().append($("<h1>").addClass("my-3").text("🎵⛔ Lyric is unavailable for this song. 🎵⛔"))}let i=document.querySelector("#videobg"),a=i.querySelector("source"),n=client.cdn+"/static/media/video/bg/",c=["frierendance.webm","stelle.webm","frierenvibe.webm","chottomate.webm","1.webm","2.webm"];function d(){var e=$(".current");if(!e?.length)return logger.error("No current lyric found");e[0].scrollIntoView({behavior:"smooth",block:"center",inline:"center"})}function p(t){var i=document.querySelectorAll(".song-name");for(let e=0;e<i.length;e++)i[e].textContent=t}function u(t){var i=document.querySelectorAll(".artist-name");for(let e=0;e<i.length;e++)i[e].textContent=t}function m(e){let t="";e?t+=e.map(e=>`<h2 class='my-3' data-timestamp='${e.time}'>${e.line}</h2>`).join(""):t="<h2 class='my-3'>Lyric is unavailable for this song.</h2>",s.html(t),s.find("h2").off("click").on("click",function(){var e=$(this).data("timestamp");e=e/1e3,$("#jplayer_N").jPlayer("play",e)})}async function y(e){var t,i=s.find("h2");i?.length&&(t=await i.filter(function(){return $(this).data("timestamp")<=e}).last(),i.removeClass("current"),t.addClass("current"))}function h(e){let s=[],a="https://spotify-lyrics-api.nnsvn.me/?url="+e;return new Promise((t,i)=>{$.ajax({url:a,type:"GET",success:async function(e){let a=e.lines;if(!a)throw new Error("No lyrics found");await a.map((e,t)=>{var i=parseInt(e.startTimeMs),t=(a[t+1]&&parseInt(a[t+1].startTimeMs),e.words);s.push({time:i,line:t})}),t(s)},error:function(e){i(e)}})})}$("#replacebg").on("click",function(){let e;for(;(e=n+c[Math.floor(Math.random()*c.length)])===a.getAttribute("src"););a.setAttribute("src",e),i.load(),i.play()});var e=window.UrlHandler.getQuery("guildId");if(e)$("#lyrics").css("height","70vh"),$.ajax({url:"/musicplayer/"+e,type:"GET",success:async function(e){let a=e.data;if(!Object.keys(a).length)return logger.error("No data found"),o(s);p(a.songDetails.name),u(a.songDetails.artists[0].name),await h(a.songDetails.external_urls.spotify).then(async e=>{if(e){await m(e);let i=moment(a.timeStarted);if(i.isValid()){let t=setInterval(()=>{var e=Date.now(),e=moment(e).diff(i,"seconds");y(1e3*(e+a.currentTime)),d(),e+a.currentTime>=a.songDetails.duration_ms/1e3&&clearInterval(t)},200)}s.find("h2").off("click").on("click",()=>{var e=$(this).data("timestamp")/1e3;i=moment(Date.now()).subtract(e,"seconds")})}}).catch(e=>{window.console.error(e),o(s)})},error:function(e){window.console.error(e),o(s)}});else if(window.UrlHandler.getQuery("musicplayer")){setInterval(()=>{l&&r&&d()},200);$("#player_container").append(`
        <div id="player" class="rounded-3 rounded-top-0">
          ...
          </div>`),$(".float-song-card").off("click").on("click",function(){var e;e=$(this).attr("data-index"),a.play(e),$(".jp-play-me").removeClass("active"),$(".jp-play-me").parent("li").removeClass("active"),$(`.jp-play-me[data-index="${e}"]`).addClass("active"),$(`.jp-play-me[data-index="${e}"]`).parent("li").addClass("active")})}),new jPlayerPlaylist({jPlayer:"#jplayer_N",cssSelectorAncestor:"#jp_container_N"},i,e));$("#playlist").on("shown.bs.dropdown",function(){$(".mplayer .jp-playlist.open ul").css({left:"auto",right:"-21px",width:"300px"})}),$(".mplayer").css("display",""),$(document).on($.jPlayer.event.timeupdate,a.cssSelector.jPlayer,function(e){y(t=(e=e.jPlayer.status.currentTime,parseInt(1e3*e)))}),$(document).on($.jPlayer.event.play,a.cssSelector.jPlayer,function(e){$(".musicbar").addClass("animate"),l=!0}),$(document).on($.jPlayer.event.pause,a.cssSelector.jPlayer,function(e){$(".musicbar").removeClass("animate"),$(".jp-play-me").removeClass("active"),$(".jp-play-me").parent("li").removeClass("active"),l=!1}),$(document).on($.jPlayer.event.ended,a.cssSelector.jPlayer,function(e){$(".musicbar").removeClass("animate"),$(".jp-play-me").removeClass("active"),$(".jp-play-me").parent("li").removeClass("active"),t=0,l=!1}),$(document).on($.jPlayer.event.setmedia,a.cssSelector.jPlayer,function(t){p(t.jPlayer.status.media.title),u(t.jPlayer.status.media.artist),e=t.jPlayer.status.media.poster,$("#album-art").attr("src",e);var e=i.find(e=>e.title===t.jPlayer.status.media.title);e?.lyrics?(e=e.lyrics,$.getJSON(e).then(function(e){return e?.lyrics?e.lyrics:null}).then(e=>{r=!0,m(e)}).catch(e=>{r=!1,window.console.error(e),o(s)})):o(s)})}else{$("#lyrics").css("height","70vh");$("#player_container").append(`
    ...
    `),$("#search_lyrics_btn").on("click",async function(){var e,t=$("#search_lyrics").val();if(!t)return logger.error("No query provided");e=t,new Promise((t,i)=>{$.ajax({url:"/musicplayer/search",type:"POST",contentType:"application/json",data:JSON.stringify({query:e}),success:function(e){t(e)},error:function(e){i(e)}})}).then(async e=>e.error?logger.error(e.error):(e=e.track)?(p(e.name),u(e.artists[0].name),void await h(e.external_urls.spotify).then(async e=>{e&&await m(e)}).catch(e=>{window.console.error(e),o(s)})):logger.error("No track found")).catch(e=>{window.console.error(e),o(s)})})}})();