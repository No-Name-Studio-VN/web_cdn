$(function(){let i=document.getElementById("lvlModal");i.addEventListener("show.bs.modal",a=>{a=a.relatedTarget;a.classList.add("active","modalactive");let n=a.getAttribute("data-bs-id");a=i.getAttribute("data-bs-id");if(n!=a){let t=i.querySelector(".modal-body");a=i.querySelector(".modal-footer");t.innerHTML=`<div class="d-flex justify-content-center"><div class="spinner-border" role="status"><span class="visually-hidden">${i18next.t("dashboard.loading")}...</span></div></div>`,$.ajax({url:`/dashboard/${window.location.pathname.split("/")[2]}/${window.location.pathname.split("/")[3]}/user/`+n,method:"GET",contentType:"application/json",success:function(a){var a=a.data,e=`
        ...
        </div>`;t.innerHTML=e,i.setAttribute("data-bs-id",n),document.getElementById("leaderboardform").setAttribute("action",`/dashboard/${a.guildId}/leaderboard/user/`+a.userId)},error:function(a){t.innerHTML=window.ContentHandler.createAlert(a.responseJSON.error)}}),a.innerHTML=`
