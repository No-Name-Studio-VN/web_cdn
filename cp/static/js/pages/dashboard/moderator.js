$("#automod-tab").one("show.bs.tab",function(){$.ajax({url:`/dashboard/${window.location.pathname.split("/")[2]}/${window.location.pathname.split("/")[3]}/automodrules`,method:"GET",contentType:"application/json",success:function(l){for(let t=0;t<l.data.length;t++){var c=l.data[t];let e,a,s=(4===c.triggerType?(e="blockwords",a=`
          ...
          `;$("#accordion").append(c)}},error:function(e){console.log(e.responseJSON.error)}});const s=document.getElementById("modal");s.addEventListener("show.bs.modal",e=>{const t=e.relatedTarget,l=t.getAttribute("data-bs-id"),a=(s.querySelector(".modal-body").innerHTML=`Are you sure you want to ${t.getAttribute("btn-type")} the "${t.getAttribute("data-bs-name")}" AutoMod rule?`,s.querySelector(".modal-footer").innerHTML=`<button type="button" class="btn btn-dark" data-bs-dismiss="modal">${i18next.t("dashboard.cancel")}</button>
